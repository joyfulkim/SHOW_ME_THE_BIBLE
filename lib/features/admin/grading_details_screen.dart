...
class GradingDetailsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final submissionsAsync =
        ref.watch(submissionsStreamProvider(sessionId, roundNumber));
        ...
        child: Column(
          ...
          children: [
                  ...
                  child: Text(
                    () {
                      ...
                      if (!isFinal &&
                        ...
                        score = AccuracyCalculator.calculate(
                            widget.originalText, widget.submission.content);
                        return '실시간 정확도: ${score.toStringAsFixed(1)}%';
                      }

                      return isFinal
                          ? '최종 점수: ${score.toStringAsFixed(1)}%'
                          : '현재 정확도: ${score.toStringAsFixed(1)}%';
                    }(),
                    style: const TextStyle(
                        color: AppTheme.kNavy,
                        fontWeight: FontWeight.bold,
            ...
            const _SectionHeader(
                icon: Icons.edit_note, title: '참가자 제출 답안 (비교)'),
            const Gap(12),
            Container(
              width: double.infinity,
              padding: const EdgeInsets.all(20),
            ...
            Row(
              children: [
                ...
                const Gap(12),
                Text('${_customScore.round()}점',
                    style: const TextStyle(fontWeight: FontWeight.bold)),
                IconButton(
                  onPressed: () =>
                      _updateScore(_customScore.round().toDouble()),
  ...
  Future<void> _updateScore(double score) async {
    try {
      await ref
          ...
          .updateSubmissionScore(widget.submission.id, score);
      if (mounted) {
